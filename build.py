...
@task()
def build():
  nsh.docker('build --rm=true -t alpine-haproxy-socklog .'.split(' '))
...
def push():
  ...
  if Travis().is_tag():
    print('Pushing navioonline/alpine-haproxy-socklog:{tag}'.format(tag=Travis().tag()))
    nsh.docker.tag(
        'alpine-haproxy-socklog:latest', 
        'navioonline/alpine-haproxy-socklog:{tag}'.format(tag=Travis().tag())
      )
    nsh.docker.push(
        'navioonline/alpine-haproxy-socklog:{tag}'.format(tag=Travis().tag())
      )
  else:
    print('Pushing navioonline/alpine-haproxy-socklog:{tag}'.format(tag=Config[branch]['docker_tag']))
    nsh.docker.tag(
        'alpine-haproxy-socklog:latest', 
        'navioonline/alpine-haproxy-socklog:{tag}'.format(tag=Config[branch]['docker_tag'])
      )
    nsh.docker.push(
        'navioonline/alpine-haproxy-socklog:{tag}'.format(tag=Config[branch]['docker_tag'])
      )
